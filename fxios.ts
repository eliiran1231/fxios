...
export class Fxios{
  ...
  constructor(){
    ...
    addmember(username: string, password: string){
    const data = querystring.stringify({
      username: username ,
      password: "" ,
      passwordconfirm: "" ,
    ...
    sendMessage(showtherdId:number, message:string){
      var securitytoken = this.info.securitytoken;
        const data = querystring.stringify({
          securitytoken: securitytoken,
          ajax: "1",
    ...
    newthread(forumId:number, tag:number, title:string, content:string){
      var securitytoken = this.info.securitytoken;
        const data = querystring.stringify({
          prefixid: tag + "",
          subject: title + "",
    ...
    deleteMessage(commentId:number){
      var securitytoken = this.info.securitytoken;
        const data = querystring.stringify({
          s: "",
          securitytoken: securitytoken,
    ...
    editMessage(commentId:number, content:string){
      var securitytoken = this.info.securitytoken;
        const data = querystring.stringify({
          securitytoken: securitytoken,
          do: "updatepost",
    ...
    sendNewPM(user:number, subject:string, message:string){
      var securitytoken = this.info.securitytoken;
        const data = querystring.stringify({
          securitytoken: securitytoken,
          do: "insertpm",
    ...
    sendPM(pmId:number, user:number, message:string){
    var securitytoken = this.info.securitytoken;
      const data = querystring.stringify({
        message: message + "",
        fromquickreply: "1",
    ...
    async getQouteInfo(commentId:number):Promise<Message>{
      const data = querystring.stringify({
      ...
      const user:User = await this.getUserInfoByName(username);
     ...
     return new Promise<Message>(resolve => resolve(message)).catch(reject => reject("there was an error"))               
    ...
    socket.on('newreply',async(data)=>{
      ...
      const message:Message = {
        ...
        reply: (msg:string)=>{this.sendMessage(data.thread_id, VBQuote() + msg)}
      }
    ...
    onNewLike(callback:(like:Like)=>void){
  var socket = io.connect('https://socket5.fxp.co.il');
  socket.on('connect',()=>{
  var send = this.info.send;
  socket.send(send)
    ...
    onNewPM(callback:(pm:PM)=>void){
      ...
      socket.on('newpmonpage',async(data)=>{
        const pm:PM = {
          ...
          reply: (msg:string)=>{this.sendPM(data.pmid,data.username,msg)} 
        }
        callback(pm)
    })
  
  }
    async onNewThread(forumId:number, callback:(thread:Thread)=>void){
    var socket = io.connect('https://socket5.fxp.co.il');
  ...
  onNewMessageOnThread(thread_id:number,callback:(msg:Message)=>void){
    var socket = io.connect('https://socket5.fxp.co.il');
    socket.on('connect',async()=>{
